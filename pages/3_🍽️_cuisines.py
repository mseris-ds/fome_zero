...
def max_rating_cuisines( df1, tipo ):
    df_aux = (df1.loc[(df1.loc[: , 'cuisines'] == tipo) , ['restaurant_name' , 'aggregate_rating', 'restaurant_id']]
    ...
    label = f'{df_aux.iloc[0,5]} : {df_aux.iloc[0,0]}'
    value = f'{df_aux.iloc[0,6]}/5.0'
    
    
    return label, value    

def top10_restaurants( df ):
...
restaurant_numbers = st.sidebar.slider('Selecione a quantidade de Restaurantes que deseja visualizar', 1, 20, 10)
...
with st.container():
    ...
    with col1:
        ...
        label, value = max_rating_cuisines( df1, 'Italian' )
        col1.metric(label, value)

    with col2:
        
        label, value = max_rating_cuisines( df1, 'American' )
        col2.metric(label, value)
        
    with col3:

        label, value = max_rating_cuisines( df1, 'Arabian' )
        col3.metric(label, value)

    with col4:

        label, value = max_rating_cuisines( df1, 'Japanese' )
        col4.metric(label, value)

    with col5:
        
        label, value = max_rating_cuisines( df1, 'Brazilian' )
...
with st.container():
    ...
    with col1:
        ...
        fig = top10_best_cuisines( df2 )
        st.plotly_chart(fig, use_container_width = True)

    with col2:

        fig = top10_worst_cuisines( df2 )
        st.plotly_chart(fig, use_container_width = True)
